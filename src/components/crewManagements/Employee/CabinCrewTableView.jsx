...
// import actions
import { deleteEmployee, getCabinCrewSearchList, getContactTypeList } from '../../../services/actions/CrewManagementActions/employeeAction'
import { getAircraftTypeList } from '../../../services/actions/FlightManagementActions/aircraftTypeAction'

// import config
import { PAGE_SIZE } from '../../../config'

const CabinCrewTableView = () => {
    // get employee state values from redux
    const { isLoadingForCabin, cabinCrewList, pagination } = useSelector(state => state.cabinCrews)
    const { aircraftTypePermission } = useSelector(state => state.auth)
    const { aircraftTypeList } = useSelector(state => state.aircrafttype)
    const { contactTypeList } = useSelector(state => state.employee)
    ...
    return (
        <div className={'bg-white px-4 py-3 rounded-md'}>
            ...
            <ScrollTableBlock 
                action={getCabinCrewSearchList}
                columns={columns}
                dataList={cabinCrewList}
                pagination={pagination} 
                isLoading={isLoadingForCabin} 
                aircraftTypePermission={ aircraftTypePermission }
            />
        </div>
    )
}
export default CabinCrewTableView